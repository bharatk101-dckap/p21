...
def sales(company_Id, order):
    if order == 'completed':
        ...
        raise Exception('Recent Order Pull Unsuccessful')


@task
def sales_executive(company_Id, sales):
    if sales == 'completed':
        try:
            ...
            df_to_psql(company_Id=company_Id, table_name="sales_summary", df=df,
                       primary_key=None, is_primary=False, drop=True)
            wrangle_track(company_id=company_Id, method='Summary', message="Successfully completed")
            return "completed"
        except Exception as e:
            wrangle_track(company_id=company_Id, method='Summary', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Clean sales is unsuccessful")


@task
def sales_forecasting(company_Id, sales):
    if sales == "completed":
        try:
            query = "SELECT * FROM SALES;"
            sales = psql_to_df(company_Id=company_Id, query=query)
            ...
            df_to_psql(company_Id=company_Id, table_name="sales_forecasting", df=forecast,
                       primary_key="ds", drop=True)
            wrangle_track(company_id=company_Id, method='Forecasting', message="Successfully completed")
            return {"CID": company_Id, "Status": "Completed"}
        except Exception as e:
            wrangle_track(company_id=company_Id, method='Forecasting', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Clean Sales is Unsuccessful")
...
@task
def cust_segment(company_Id, sales, sales_exec):

    if sales == "completed" and sales_exec == "completed":
        ...
        try:
            ...
            df = psql_to_df(company_Id, df_query)
            s_df = psql_to_df(company_Id, s_df_query)
            # expected_order = psql_to_df(company_Id, epc_o_query)
            ...
            df_to_psql(company_Id=company_Id, table_name="rfm", df=rfm, primary_key="email",
                       drop=True)
            wrangle_track(company_id=company_Id, method='RFM', message="Successfully completed")
            return "completed"
        except Exception as e:
            wrangle_track(company_id=company_Id, method='RFM', message=e)
            return {"CID": company_Id, "Status": e}
    elif sales != "completed":
        raise Exception("Clean Sales is Unsuccessful")
    elif sales_exec != "completed":
        raise Exception("Clean Sales Executive is Unsuccessful")
    else:
        raise Exception("Both Clean Sales & Clean Sales Executive is Unsuccessful")


@task
def cust_segment_one(company_Id, sales, sales_exec):
    if sales == "completed" and sales_exec == "completed":
        try:
            df_query = "SELECT customer_id, email, just_date, order_id, total FROM sales;"
            s_df_query = "SELECT email, min_c, max_c FROM sales_summary;"
            ...
            # rfm_expected_order.set_index('email')
            #
            # # rfm.to_csv(directory + '/cust_segment.csv')
            # rfm_expected_order.reset_index(inplace=True)
            # # rfm_expected_order.next_order_date.replace({'NaT': None}, inplace=True)
            # rfm_expected_order.next_order_date.replace({np.nan: None}, inplace=True)
            df_to_psql(company_Id=company_Id, table_name="rfm_one", df=rfm, primary_key="email",
                       drop=True)
            wrangle_track(company_id=company_Id, method='RFM One', message="Successfully completed")
            return {"CID": company_Id, "Status": "RFM Single Completed"}
        except Exception as e:
            wrangle_track(company_id=company_Id, method='RFM One', message=e)
            return {"CID": company_Id, "Status": e}
    elif sales != "completed":
        raise Exception("Clean Sales is Unsuccessful")
    elif sales_exec != "completed":
...
@task
def cohort_six(company_Id, t_day, p_day, sales):
    if sales == "completed":
        try:
            query = "SELECT order_id, date_created, email, total FROM sales where date_created >= '" \
                    + str(p_day) + "' and date_created <='" + str(t_day) + "'"
            df = psql_to_df(company_Id, query)
            # df = df.compute()
            ...
            d = datetime.datetime.strptime(f"{t.year}-{t.month}-1", "%Y-%m-%d").date() - relativedelta(months=6)
            ...
            # print(retention)
            # cohort_counts.to_csv(directory + '/cohort_half.csv')
            # retention.to_csv(directory + '/retention_half.csv')
            cohort_counts.reset_index(inplace=True)
            df_to_psql(company_Id=company_Id, table_name="cohort_half", df=cohort_counts,
                       primary_key='CohortMonth',
                       drop=True)
            retention.reset_index(inplace=True)
            df_to_psql(company_Id=company_Id, table_name="retention_half", df=retention,
                       primary_key='CohortMonth', drop=True)
            wrangle_track(company_id=company_Id, method='Cohort 6', message="Successfully completed")
            return {"CID": company_Id, "Status": "Cohort six Completed"}
        except Exception as e:
            wrangle_track(company_id=company_Id, method='Cohort 6', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Clean Sales is Unsuccessful")
...
@task
def cohort_year(company_Id, t_day, p_day, sales):
    if sales == "completed":
        try:
            query = "SELECT order_id, date_created, email, total FROM sales where date_created >= '" \
                    + str(p_day) + "' and date_created <='" + str(t_day) + "'"
            ...
            # cohort_counts.to_csv(directory + '/cohort_year.csv')
            # retention.to_csv(directory + '/retention_year.csv')

            cohort_counts.reset_index(inplace=True)
            df_to_psql(company_Id=company_Id, table_name="cohort_year", df=cohort_counts,
                       primary_key='CohortMonth', drop=True)
            retention.reset_index(inplace=True)
            df_to_psql(company_Id=company_Id, table_name="retention_year", df=retention,
                       primary_key='CohortMonth', drop=True)
            wrangle_track(company_id=company_Id, method='Cohort 1', message="Successfully completed")
            return {"CID": company_Id, "Status": "Completed"}
        except Exception as e:
            wrangle_track(company_id=company_Id, method='Cohort 1', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Clean Sales is Unsuccessful")
...
@task
def summ_rfm(company_Id, sales_exec, cust_segment):

    if sales_exec == "completed" and cust_segment == "completed":

        try:
            summ_query = "SELECT * FROM sales_summary;"
            rfm_query = "SELECT email,  cust_segment, quad_segment FROM rfm;"
            summ = psql_to_df(company_Id, summ_query)
            rfm = psql_to_df(company_Id, rfm_query)
            df = pd.merge(rfm, summ, on='email', how='outer', validate="one_to_many")
            df_to_psql(company_Id=company_Id, df = df, table_name="summ_rfm", primary_key=None, is_primary=False,
                       drop=True)
            wrangle_track(company_id=company_Id, method='Summary RFM Clean', message="Successfully completed")
            return "completed"

        except Exception as e:
            wrangle_track(company_id=company_Id, method='Summary RFM Clean', message=e)
            return {"CID": company_Id, "Status": e}
    elif cust_segment != "completed":
        raise Exception("Clean Customer Segmentation is Unsuccessful")
    elif sales_exec != "completed":
        raise Exception("Clean Sales Executive is Unsuccessful")
    else:
        raise Exception("Both Clean Customer Segmentation & Clean Sales Executive is Unsuccessful")


@task
def churn_forecasting(company_Id, sales_exec):
    """
    """
    if sales_exec == "completed":
        try:
            ...
            df_to_psql(company_Id=company_Id, table_name="churn_forecasting", df=forecast, primary_key="ds",
                       drop=True)
            wrangle_track(company_id=company_Id, method='Churn_Forecasting', message="Successfully completed")
            return {"CID": company_Id, "Status": "Completed"}

        except Exception as e:
            wrangle_track(company_id= company_Id, method='Churn_Forecasting', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Clean Sales Executive is Unsuccessful")
...
@task
def orders_forecasting(company_Id, sales_exec):
    if sales_exec == "completed":
        try:
            """
            ...
            df_to_psql(company_Id=company_Id, table_name="order_forecasting", df=forecast,
                       primary_key="ds", drop=True)
            wrangle_track(company_id=company_Id, method='Order Forecasting', message="Successfully completed")
            return {"CID": company_Id, "Status": "Completed"}
        except Exception as e:
            wrangle_track(company_id=company_Id, method='Order Forecasting', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Clean Sales Executive is Unsuccessful")
...
@task
def vendor_spend_analysis(company_Id, vendor):
    if vendor == "completed":
        try:
            query = "SELECT * FROM purchase_order"
            df = psql_to_df(company_Id, query)
            a = pd.DataFrame(df.groupby(["item_description"])["price"].sum())
            a = a.sort_values(by=["price"], ascending=True)
            a['price'] = round(100*(a.price.cumsum() / a.price.sum()), 2)
            a = a.sort_values(by=["price"], ascending=False)
            a["spend_class"] = a.apply(lambda x: "Major Spend" if x.price > 20 else "Tail Spend", axis=1)
            a.reset_index(inplace=True)
            a = a[["item_description", "spend_class"]]
            df["contract"] = df.apply(lambda x: "Contract Not Available" if not x.contract_number else "Contract Available", axis=1)
            df["po"] = df.apply(lambda x: "PO Not Available" if str(x.po_no) == "nan" else "PO Available", axis=1)
            df = df.merge(a, on="item_description", left_index=True)
            b = pd.DataFrame(df.groupby(["item_description"])["vendor_name"].count())
            b.rename(columns={"vendor_name": "no_of_vendors"}, inplace=True)
            b.reset_index(inplace=True)
            df = df.merge(b, on="item_description")
            df["vendor_class"] = None
            df["vendor_class"][df["no_of_vendors"] == 1] = "Single Vendor"
            df["vendor_class"][(df["no_of_vendors"] > 1) & (df["no_of_vendors"] <= 5)] = "Between 1 and 5"
            df["vendor_class"][df["no_of_vendors"] > 5] = "More than 5"
            c = pd.DataFrame(df[df["po"] == "PO Available"].groupby(["item_description"])["po_no"].count())
            c.rename(columns={"po_no": "no_of_pos"}, inplace=True)
            c.reset_index(inplace=True)
            df = df.merge(c, on="item_description")
            df["po_class"] = None
            df["po_class"][df["no_of_pos"] == 1] = "Single PO"
            df["po_class"][(df["no_of_pos"] > 1) & (df["no_of_pos"] <= 12)] = "Between 1 and 12"
            df["po_class"][df["no_of_pos"] > 12] = "More than 12"
            df = df.sort_values(by="date_created")
            df_to_psql(company_Id=company_Id, table_name="vendor_spend_analysis",
                       df=df, primary_key="index", is_primary=True, drop=True)
            return "completed"
        except Exception as e:
            wrangle_track(company_id=company_Id, method='vendor_spend_analysis', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Recent pull Vendor is Unsuccessful")


@task
def vendor_deals(company_Id, vendor_spend):
    if vendor_spend == "completed":
        try:
            query = "SELECT * FROM vendor_spend_analysis"
            df = psql_to_df(company_Id, query)
            grouper17 = \
            df[df["contract"] == "Contract Not Available"].groupby(["item_description", "vendor_name", "year"])[
                "unit_price"]
            grouper18 = \
            df[df["contract"] == "Contract Not Available"].groupby(["item_description", "vendor_name", "year"])["price"]
            vendor_bad_deals = pd.concat([grouper17.mean(), grouper18.sum()], axis=1).reset_index()
            vendor_bad_deals = vendor_bad_deals.round({"price": 1, "unit_price": 1})
            vendor_bad_deals["disc_factor"] = 1
            vendor_bad_deals["adjusted_price"] = vendor_bad_deals["unit_price"] * vendor_bad_deals["disc_factor"]
            grouper19 = vendor_bad_deals.groupby(["item_description"])["unit_price"]
            grouper20 = vendor_bad_deals.groupby(["item_description"])["year"]
            min_ = grouper19.min().rename("min_price").reset_index()
            max_ = grouper19.max().rename("max_price").reset_index().drop(columns="item_description")
            cnt_ = grouper19.count().rename("item_count").reset_index().drop(columns="item_description")
            min_year = grouper20.min().rename("min_year").reset_index().drop(columns="item_description")
            vendor_bad_deals = vendor_bad_deals.merge(pd.concat([min_, max_, cnt_, min_year], axis=1),
                                                      on=["item_description"])
            vendor_bad_deals = vendor_bad_deals.sort_values(by=["item_count", "vendor_name", "year"],
                                                            ascending=False).drop(columns="item_count")
            vendor_bad_deals["ppv"] = vendor_bad_deals.apply(
                lambda x: int(round(x.max_price / x.min_price * 100, 0)) if x.min_price > 0 else 0, axis=1)
            vendor_bad_deals["vendor_price_index"] = vendor_bad_deals.apply(
                lambda x: int(round(x["adjusted_price"] / x.min_price * 100, 0)) if x.min_price > 0 else 0, axis=1)
            vendor_bad_deals["Purchase_Period"] = vendor_bad_deals.apply(
                lambda x: "1st Year of Purchase for the item" if x.year == x.min_year else x.year, axis=1)
            vendor_bad_deals["best_quote_score"] = vendor_bad_deals.apply(
                lambda x: 1 if x["vendor_price_index"] == 100 else 0, axis=1)
            vendor_bad_deals["range_selection"] = vendor_bad_deals.apply(
                lambda x: "> 50 times - Exclude" if x["vendor_price_index"] > 5000 else "Include", axis=1)
            vendor_bad_deals["extra_cost"] = vendor_bad_deals.apply(
                lambda x: round((x.price / x.unit_price) * (x["adjusted_price"] - x.min_price),
                                1) if x.min_price > 0 else 0, axis=1)
            vendor_bad_deals.reset_index(drop=True, inplace=True)
            vendor_bad_deals["s_no"] = vendor_bad_deals.index
            grouper21 = \
            vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["item_description"])[
                "min_price"]
            grouper22 = \
            vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["item_description"])[
                "unit_price"]
            ...
            vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["item_description"])[
                "vendor_price_index"]
            grouper24 = \
            vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["item_description"])["ppv"]
            grouper25 = \
            vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["item_description"])[
                "range_selection"]
            grouper26 = \
            vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["item_description"])[
                "extra_cost"]
            grouper27 = \
            vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["item_description"])["price"]
            vendor_deals_1 = pd.concat(
                [grouper21.mean().rename("avg_min_price_itm"), grouper22.mean().rename("avg_unit_price_itm"),
                 grouper23.mean().rename("avg_vendor_price_index_itm"), grouper24.mean().rename("avg_ppv_itm"),
                 grouper25.count().rename("cnt_range_itm"), grouper26.sum().rename("tot_extra_cost_itm"),
                 grouper27.sum().rename("tot_price_itm")], axis=1).reset_index()
            vendor_bad_deals = vendor_bad_deals.merge(vendor_deals_1, on="item_description")
            grouper28 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "min_price"]
            grouper29 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "unit_price"]
            grouper30 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "vendor_price_index"]
            grouper31 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "ppv"]
            grouper32 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "best_quote_score"]
            grouper33 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "range_selection"]
            grouper34 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "extra_cost"]
            grouper35 = vendor_bad_deals[vendor_bad_deals["range_selection"] == "Include"].groupby(["vendor_name"])[
                "price"]
            vendor_deals_2 = pd.concat([grouper28.mean().rename("avg_min_price_vnd").round(1),
                                        grouper29.mean().rename("avg_unit_price_vnd").round(1),
                                        grouper30.mean().rename("avg_vendor_price_index_vnd").round(0),
                                        grouper31.mean().rename("avg_ppv_vnd").round(0),
                                        grouper32.sum().rename("sum_score").round(1),
                                        grouper33.count().rename("cnt_range_vnd").round(0),
                                        grouper34.sum().rename("tot_extra_cost_vnd").round(1),
                                        grouper35.sum().rename("tot_price_vnd").round(1)], axis=1).reset_index()
            vendor_bad_deals = vendor_bad_deals.merge(vendor_deals_2, on="vendor_name")
            vendor_bad_deals["good_score"] = vendor_bad_deals.apply(
                lambda x: int(round((x.sum_score / x.cnt_range_vnd) * 100, 0)), axis=1)
            vendor_bad_deals.sort_values(by=["cnt_range_itm", "vendor_price_index", "item_description", "vendor_name"],
                                         ascending=False, inplace=True)
            vendor_bad_deals.reset_index(drop=True, inplace=True)
            df_to_psql(company_Id=company_Id, table_name="vendor_deals", df=vendor_bad_deals,
                       primary_key="index", is_primary=True, drop=True)
        except Exception as e:
            wrangle_track(company_id=company_Id, method='vendor_deals', message=e)
            return {"CID": company_Id, "Status": e}
    else:
        raise Exception("Clean Vendor Spend Analysis is Unsuccessful")
